...
class ScanRepository:
    ...
    async def create(self, scan_job: ScanJob) -> ScanJob:
        ...
        async with await get_db_connection() as db:
            ...
            """, (
                scan_job.id,
                scan_job.filename,
                scan_job.status.value,
                scan_job.created_at.isoformat()
            ))
            await db.commit()
        return scan_job
    ...
    async def update_status(self, scan_id: str, status: ScanStatus, error: Optional[str] = None):
        ...
        async with await get_db_connection() as db:
            ...
            """, (status.value, error, scan_id))
            await db.commit()
    
    async def update_completion(self, scan_id: str, status: ScanStatus, findings_count: int):
...
class FindingRepository:
    ...
    async def get_by_job_id(
        ...
        query = """
            SELECT id, job_id, file_path, line_number, secret_type, secret,
                   severity, rule_id, confidence, created_at
            FROM findings 
            WHERE job_id = ?
        """
    ...
    async def create(self, repository: Repository) -> Repository:
        ...
        async with await get_db_connection() as db:
            await db.execute("""
                ...
                (id, url, provider, name, token, status, webhook_secret, 
                 discord_webhook_url, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                repository.id,
                str(repository.url),
        ...
        async with await get_db_connection() as db:
            cursor = await db.execute("""
                SELECT id, url, provider, name, token, status, webhook_secret,
                       discord_webhook_url, last_scan, last_scan_status, 
            ...
            if row:
                ...
                return Repository(
                    ...
                    last_scan_status=row[9],
                    findings_count=row[10],
